...
object Plugin {
  lazy val defaultSettings = inConfig(Scct)(Defaults.compileSettings) ++ inConfig(ScctTest)(Defaults.testSettings) ++ Seq(
    ...
    sbt.Keys.test in Scct <<= (sbt.Keys.test in ScctTest),
  ...
  // Report mergin':
  lazy val mergeReportSettings = Seq(
    scctReportDir <<= crossTarget / "coverage-report",
    scctMergeReportSourceFiles <<= (thisProjectRef, buildStructure) map coverageResultFiles,
    scctMergeReport <<= (scctMergeReportSourceFiles, scctReportDir) map generateReport)
